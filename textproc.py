# textproc.py — evidence structuring + token budgeting (ported from the fork)
# Turns raw fetched source text into deduplicated, relevance-ranked sentences
# and packs them into a token budget for the LLM analysis call.
import functools
import re
...
BOILERPLATE_SENTENCE_PATTERNS = [
    ...
    re.compile(r"(all rights reserved|copyright \(c\)|©\s*\d{4})", re.IGNORECASE),
...
def normalize_currency_mentions(text: str) -> str:
    def _rewrite(match):
        ...
        if unit.startswith("cr"):
            ...
            unit_label = ""
        return ("₹" + amount + " " + unit_label).strip() + " "

    text = _CURRENCY_NORMALIZE_PATTERN.sub(_rewrite, text)
    text = _BARE_CRORE_PATTERN.sub(lambda m: "₹" + m.group(1) + " crore ", text)
    return text
...
def _clause_safe_ngrams(normalized_text: str, n: int) -> list:
    ngrams = []
...
def extract_keywords(text: str, top_n: int = 40, document_frequency: Counter = None, corpus_size: int = 1) -> list:
...
def score_sentence_relevance(sentence: str, company_tokens: list) -> float:
...
def company_name_tokens(company: str) -> list:
...
def structure_source_text(source_name: str, raw_text: str, company: str, max_sentences: int = 40,
                           document_frequency: Counter = None, corpus_size: int = 1,
                           global_seen_keys: set = None) -> dict:
...
def structure_all_sources(sources: list, company: str, max_sentences_per_source: int = 25) -> list:
...
def build_token_budgeted_evidence(structured_sources: list, company: str, token_budget: int) -> str:
    ...
    header = "KEY TERMS ACROSS SOURCES: " + ", ".join(top_global_keywords) + "\n\n"
    ...
    block_lines_by_source = {s["source_name"]: ["[" + s["source_name"] + "]"] for s in ordered_sources}
...
def build_sources_manifest(sources: list) -> str:
    """One line per source: name | status | url — used by the LLM to cite."""
    lines = []
    for s in sources:
